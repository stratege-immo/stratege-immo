...
<a href="https://stratege-immo.fr/index.html#simulation"
   style="background:#3DD9BE;color:#1E3040;padding:14px 32px;border-radius:999px;text-decoration:none;font-weight:600;font-size:15px;display:inline-block">
Lancer ma première simulation</a></div>`);
}

function simulationEmail(prenom, r) {
  return wrap(`
<h2 style="color:#1E3040;font-family:Georgia,serif;margin:0 0 16px">Votre simulation, ${prenom}</h2>
<p style="color:#3D566A;font-size:16px;line-height:1.6;margin:0 0 24px">
Voici le récapitulatif de votre simulation d'investissement immobilier.
</p>
<table width="100%" cellpadding="0" cellspacing="0" style="margin:0 0 24px">
<tr><td style="padding:12px 16px;background:#f5f9fb;border-radius:8px 8px 0 0;font-size:14px;color:#7FA3B9">Budget total</td>
<td style="padding:12px 16px;background:#f5f9fb;text-align:right;font-weight:700;color:#1E3040;font-size:18px">${fmt(r.budget_total)} €</td></tr>
<tr><td style="padding:12px 16px;font-size:14px;color:#7FA3B9">Mensualité</td>
<td style="padding:12px 16px;text-align:right;font-weight:600;color:#1E3040">${fmt(r.mensualite)} €/mois</td></tr>
<tr><td style="padding:12px 16px;background:#f5f9fb;font-size:14px;color:#7FA3B9">Loyer estimé</td>
<td style="padding:12px 16px;background:#f5f9fb;text-align:right;font-weight:600;color:#1E3040">${fmt(r.loyer_mensuel_estime)} €/mois</td></tr>
<tr><td style="padding:12px 16px;font-size:14px;color:#7FA3B9">Effort réel</td>
<td style="padding:12px 16px;text-align:right;font-weight:700;color:#3DD9BE;font-size:18px">${fmt(r.effort_reel)} €/mois</td></tr>
<tr><td style="padding:12px 16px;background:#f5f9fb;font-size:14px;color:#7FA3B9">Économie fiscale</td>
<td style="padding:12px 16px;background:#f5f9fb;text-align:right;font-weight:600;color:#1E3040">${fmt(r.economie_fiscale_annuelle)} €/an</td></tr>
<tr><td style="padding:12px 16px;font-size:14px;color:#7FA3B9;border-radius:0 0 8px 8px">Dispositif</td>
<td style="padding:12px 16px;text-align:right;font-weight:600;color:#1E3040;border-radius:0 0 8px 8px">${r.dispositif}</td></tr>
</table>
<div style="text-align:center;margin:24px 0">
<a href="https://stratege-immo.fr/dashboard.html"
   style="background:#3DD9BE;color:#1E3040;padding:14px 32px;border-radius:999px;text-decoration:none;font-weight:600;font-size:15px;display:inline-block">
Voir dans mon espace</a></div>
<p style="color:#7FA3B9;font-size:13px;margin:24px 0 0;font-style:italic">
...
function contactEmail(prenom, reference) {
  return wrap(`
<h2 style="color:#1E3040;font-family:Georgia,serif;margin:0 0 16px">Demande enregistrée !</h2>
<p style="color:#3D566A;font-size:16px;line-height:1.6;margin:0 0 24px">
...
<div style="font-size:13px;color:#7FA3B9;margin-bottom:4px">Référence dossier</div>
<div style="font-size:28px;font-weight:700;color:#1E3040;font-family:Georgia,serif">${reference}</div>
</div>`);
}
...
<a href="${resetLink}"
   style="background:#3DD9BE;color:#1E3040;padding:14px 32px;border-radius:999px;text-decoration:none;font-weight:600;font-size:15px;display:inline-block">
Réinitialiser</a></div>
<p style="color:#7FA3B9;font-size:14px;margin:0">Si vous n'avez pas fait cette demande, ignorez cet email.</p>`);
}
